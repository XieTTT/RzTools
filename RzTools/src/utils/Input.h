...
public:
	...
	static void     PressKey(HKey key);

	static void     KeyUp(HKey key);
	static void     KeyDown(HKey key);

	/// Checks if a key was pressed recently. Only one caller can use this function for a given key.
	static bool     WasKeyPressed(HKey key);

	/// Checks if the key is held down.
	static bool     IsKeyDown(HKey key);
	...
	static void     PressRightClick();

	static void Move(int x, int y);

	static void ClipMouse(float x, float y);

	/// Moves the cursor at the specified location, clicks and then moves the cursor back at the initial location
	static void     ClickAt(bool leftClick, float x, float y);
	...
	static void	 MoveAndPress(HKey key, float x, float y);

	static void     MoveAndKeyUp(HKey key, float x, float y);

	static void     MoveAndClick(float x, float y);

	static void     BlockInputTest(float time);

	static void     ClipCursorMove(float x, float y);

	static void	clickMove(Vector2 playerV2);
